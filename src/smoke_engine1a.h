...
typedef std::function<void(int fd,int th)> on_connect_fn_t; 
...
typedef std::function<void(int th)> on_idle_fn_t;
...
struct net_t
{
  ...
  void on_idle(on_idle_fn_t cb){on_idle_cb=cb;}    
};
...
void* processing_thread(event_info_t* ei)
{
  ...
  ASSERT(on_close_cb);
#endif  
#ifdef USE_ON_IDLE_CB  
  on_idle_fn_t on_idle_cb=net->on_idle_cb;
  ASSERT(on_idle_cb);
#endif  
  ...
  while (1) {
    ...
    nfds = epoll_wait (pfd, events, MAXEVENTS_P, wd);
